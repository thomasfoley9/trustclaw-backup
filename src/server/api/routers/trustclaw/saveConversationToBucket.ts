...
import {
  reconstructMessages,
  MESSAGE_SAFETY_CAP,
} from "./agent/context/build-context";
...
export const saveConversationToBucket = protectedProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    // Cap to the most-recent regular messages (same safety bound as the agent
    // context loader). Order desc + take + reverse so a very long conversation
    // loads its newest window rather than every row since day one; distillation
    // additionally caps tool outputs and total size downstream.
    const rows = (
