...
// Hysteresis low-water mark: when the 30% trigger fires, trim oldest-first
// down to 20% rather than stopping exactly at the trigger. Trims are
// re-derived from raw rows every turn, so a stop-at-trigger frontier would
// advance a little EVERY turn and the newly-trimmed messages at the front of
// the prompt would invalidate the Anthropic prompt cache each time (~8x
// input cost in the band). Trimming to a lower target moves the frontier in
// batches every several turns; between moves the prefix is byte-stable and
// the cache breakpoint in setup.ts keeps working.
const SOFT_TRIM_LOW_WATER_RATIO = 0.2;
...
export function pruneContext(
  ...
  // Trim OLDEST-FIRST, stopping at the low-water mark. A blanket pass here
  // used to gut every old tool result the moment the 30% line was crossed,
  // which broke "what did that email say?" while the email was still on
  // screen - recent fetches must survive until space is genuinely needed.
  // The ceiling is unchanged: under sustained pressure this still converges
  // to everything-trimmed, and the hard clear plus compaction remain the
  // backstops.
  if (ratio >= SOFT_TRIM_RATIO) {
    const lowWater = charWindow * SOFT_TRIM_LOW_WATER_RATIO;
    for (let i = 0; i < protectedBoundary && totalChars > lowWater; i++) {
      const msg = result[i]!;
      if (msg.role !== "tool") continue;

      for (let j = 0; j < msg.content.length && totalChars > lowWater; j++) {
        ...
        if (outputStr.length > SOFT_TRIM_MAX_CHARS) {
          ...
          // Escape-dense outputs can re-escape LARGER than the original when
          // sliced and re-stringified - trimming must only ever shrink.
          const saved =
            outputStr.length - JSON.stringify(replacement.output).length;
          if (saved <= 0) continue;
