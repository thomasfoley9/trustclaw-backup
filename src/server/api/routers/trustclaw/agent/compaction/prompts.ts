...
function truncateToolOutput(s: string, max: number | undefined): string {
...
// opts.maxToolOutputChars bounds each persisted tool result (scraped pages,
// email bodies can be 50-100KB each). Off by default so compaction is
// unchanged; the knowledge-distillation path passes a cap so a long or
// tool-heavy conversation can't overflow the model window / route timeout.
