...
export function TrustClawChat() {
  ...
  // Only the Postgres-backed queries are fatal. The streaming pointer is an
  // optional resume optimization (Redis-backed, fails open server-side), so its
  // error must never replace the whole chat pane - we just render without a
  // resume target.
  if (conversationsQuery.error || historyQuery.error) {
    return (
      <ErrorDisplay
        message={
