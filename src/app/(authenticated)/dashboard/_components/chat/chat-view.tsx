...
export function ChatView({
  ...
  useEffect(() => {
    ...
    if (!hasNewestRow || initialMessages.length > messages.length) {
      // A stopped or interrupted run leaves its pre-created assistant row
      // empty, and trustclaw-chat filters empty rows out of initialMessages -
      // so the server page's tail is the USER turn, not the partial. Adopting
      // it wholesale would wipe the partial reply (text + tool chips) the user
      // watched stream in and chose to keep by pressing Stop. So we adopt
      // server truth for the bulk and RE-APPEND the local partial whenever the
      // server page does not already end with a non-empty assistant reply.
      // Once the after() settle persists the partial into that row, a later
      // refetch's tail IS a non-empty assistant row, so it is not re-appended
      // (no duplicate), and server content replaces the local copy verbatim.
      const assistantHasContent = (m: UIMessage | undefined): m is UIMessage =>
        !!m &&
        m.role === "assistant" &&
        m.parts.some(
          (p) =>
      ...
      setMessages((current) => {
        ...
        const merged = [...olderLocal, ...initialMessages];
        const localTail = current[current.length - 1];
        if (
          assistantHasContent(localTail) &&
          !assistantHasContent(merged[merged.length - 1])
        ) {
