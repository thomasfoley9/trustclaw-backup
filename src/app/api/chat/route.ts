...
export async function POST(request: Request) {
  ...
  // Selected model, for provider-correct error messages (undefined until setup
  // resolves it - parseAgentError then falls back to its Anthropic-first text).
  ...
  try {
    ...
    selectedModel = prepareResult.result.selectedModel;
    ...
    await db.message
      .updateMany({
        ...
        data: {
          content: [
            { type: "text", text: parseAgentError(error, { model: selectedModel }) },
          ],
        },
      })
      .catch(() => undefined);
  ...
  after(async () => {
    ...
    try {
      ...
      if (runController.signal.aborted) {
        ...
        const text = runError
          ? parseAgentError(runError, { model: selectedModel })
